...
//     (next() straight through) and the site is PUBLIC. Setting both env vars
//     puts the whole app behind a single Basic Auth login. The app is intended
//     to be publicly reachable, so the default (no creds) is open by design.
...
export function basicAuth(req, res, next) {
  ...
  // No credentials configured: auth is disabled and the site is public.
  if (!expectedUser || !expectedPass) return next();

  // Health check stays reachable without credentials for the HEALTHCHECK.
